...
| **404** Not found | any | *(no sentinel — use any absent well-formed id)* |
...
| chassi | *(published in the seed manifest)* | linked to `ABC1D23` |
| cpf (condutor) | *(published in the seed manifest)* | driver with CNH + one infraction |
| cnpj (proprietário) | *(published in the seed manifest)* | company owner |
